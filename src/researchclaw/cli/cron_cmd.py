...
@click.option("--base-url", default=None, help="Override the API base URL")
@click.pass_context
def job_state(ctx: click.Context, job_id: str, base_url: Optional[str]) -> None:
...
def _build_spec_from_cli(
    ...
    if task_type == "text":
        ...
        return {
            ...
            "schedule": schedule, "task_type": "text",
            "text": text.strip(), "dispatch": dispatch,
            "runtime": runtime, "meta": {},
        }
    if task_type == "agent":
        if not (text and text.strip()):
            raise click.UsageError("--text is required when task type is 'agent'")
        return {
            "id": "", "name": name, "enabled": enabled,
            "schedule": schedule, "task_type": "agent",
            "request": {
                "input": [{"role": "user", "type": "message", "content": [{"type": "text", "text": text.strip()}]}],
                "session_id": target_session,
                "user_id": "cron",
            },
            "dispatch": dispatch, "runtime": runtime, "meta": {},
        }
    raise click.UsageError(f"Unsupported task type: {task_type}")


@cron_group.command("create")
@click.option("-f", "--file", "file_", type=click.Path(exists=True, dir_okay=False, path_type=Path), default=None, help="JSON spec file")
@click.option("--type", "task_type", type=click.Choice(["text", "agent"], case_sensitive=False), default=None, help="Task type")
...
@click.option("--enabled/--no-enabled", default=True, help="Create as enabled or disabled")
@click.option("--mode", type=click.Choice(["stream", "final"], case_sensitive=False), default="final", help="Delivery mode")
@click.option("--base-url", default=None, help="Override the API base URL")
@click.pass_context
def create_job(
    ctx: click.Context,
    ...
    if file_ is not None:
        ...
        for value, label in [
            (task_type, "--type"), (name, "--name"), (cron, "--cron"),
            (channel, "--channel"), (target_user, "--target-user"),
            (target_session, "--target-session"),
        ]:
            if not value or (isinstance(value, str) and not value.strip()):
                raise click.UsageError(f"When creating without -f, {label} is required")
        payload = _build_spec_from_cli(
            task_type=task_type or "agent", name=name or "",
            cron=cron or "", channel=channel or DEFAULT_CHANNEL,
            target_user=target_user or "", target_session=target_session or "",
            text=text, timezone=timezone, enabled=enabled, mode=mode,
        )
    with client(base_url) as c:
...
@click.option("--base-url", default=None, help="Override the API base URL")
@click.pass_context
def delete_job(ctx: click.Context, job_id: str, base_url: Optional[str]) -> None:
...
@click.option("--base-url", default=None, help="Override the API base URL")
@click.pass_context
def pause_job(ctx: click.Context, job_id: str, base_url: Optional[str]) -> None:
    """Pause a cron job."""
    base_url = _base_url(ctx, base_url)
    with client(base_url) as c:
...
@click.option("--base-url", default=None, help="Override the API base URL")
@click.pass_context
def resume_job(ctx: click.Context, job_id: str, base_url: Optional[str]) -> None:
