...
@click.pass_context
def list_chats(ctx: click.Context, user_id: Optional[str], channel: Optional[str], base_url: Optional[str]) -> None:
...
@click.option("--base-url", default=None, help="Override API base URL")
@click.pass_context
def get_chat(ctx: click.Context, chat_id: str, base_url: Optional[str]) -> None:
...
@click.option("-f", "--file", "file_", type=click.Path(exists=True, dir_okay=False, path_type=Path), default=None, help="Create from JSON file")
...
@click.option("--channel", default=DEFAULT_CHANNEL, help=f"Channel name (default {DEFAULT_CHANNEL})")
@click.option("--base-url", default=None, help="Override API base URL")
@click.pass_context
def create_chat(ctx: click.Context, file_: Optional[Path], name: str, session_id: Optional[str], user_id: Optional[str], channel: str, base_url: Optional[str]) -> None:
    """Create a new chat."""
    base_url = _base_url(ctx, base_url)
    if file_ is not None:
        ...
        if not session_id:
            raise click.UsageError("--session-id is required for inline creation")
        if not user_id:
            raise click.UsageError("--user-id is required for inline creation")
        payload = {"id": "", "name": name, "session_id": session_id, "user_id": user_id, "channel": channel, "meta": {}}
    with client(base_url) as c:
...
@click.option("--base-url", default=None, help="Override API base URL")
@click.pass_context
def update_chat(ctx: click.Context, chat_id: str, name: str, base_url: Optional[str]) -> None:
    """Update chat name."""
    base_url = _base_url(ctx, base_url)
    with client(base_url) as c:
        r = c.get("/chats")
...
@click.option("--base-url", default=None, help="Override API base URL")
@click.pass_context
def delete_chat(ctx: click.Context, chat_id: str, base_url: Optional[str]) -> None:
