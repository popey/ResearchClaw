...
class DingTalkChannel(BaseChannel):
    ...
    async def _send_media_part_via_webhook(
        ...
        if upload_type == "video":
            pic_media_id = (
                (part.get("pic_media_id") if isinstance(part, dict) else None)
                or (part.get("picMediaId") if isinstance(part, dict) else None)
                or getattr(part, "pic_media_id", None)
                or getattr(part, "picMediaId", None)
                or ""
            ).strip()
            if pic_media_id:
                duration = (
                    ...
                    else getattr(part, "duration", None)
                )
                if duration is None:
                    duration = 1
                payload = {
    ...
    async def start(self) -> None:
        ...
        try:
            ...
            logger.warning(
                ...
                "Install with: pip install dingtalk-stream aiohttp",
                exc,
            )
            return
        self._load_session_webhook_store_from_disk()
