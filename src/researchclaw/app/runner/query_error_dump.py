...
the full traceback, request info, and agent state into a temporary JSON
...
def _request_to_dict(request: Any) -> Any:
    """Serialize request to a JSON-serializable dict."""
    if request is None:
        return None
    try:
        raw: dict[str, Any]
...
def write_query_error_dump(
    ...
    Args:
        request: The original request object (FastAPI/Pydantic model or dict)
        exc: The exception that was raised
        locals_: The locals() dict from the calling scope, used to extract
                 agent state if available

    Returns:
        Path to the temp JSON file, or None if writing failed
    """
    try:
        ...
        # Try to extract agent state for diagnostics
        agent_state = None
        ...
        # Research-specific: capture memory manager state
        memory_state = None
        memory_manager = locals_.get("memory_manager")
        if memory_manager is not None:
            try:
                if hasattr(memory_manager, "get_stats"):
                    memory_state = _safe_json_serialize(
        ...
        fd, path = tempfile.mkstemp(
            prefix="researchclaw_query_error_",
            suffix=".json",
