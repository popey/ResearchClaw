...
def get_cron_manager(request: Request) -> CronManager:
    """Get the cron manager from app state."""
...
async def get_job(job_id: str, mgr: CronManager = Depends(get_cron_manager)):
    """Get a specific cron job with its current state."""
...
async def create_job(
    ...
    """Create a new cron job. Server generates the ID."""
...
async def get_job_state(
    ...
    """Get the current runtime state of a cron job."""
    job = await mgr.get_job(job_id)
    if not job:
        raise HTTPException(status_code=404, detail="job not found")
