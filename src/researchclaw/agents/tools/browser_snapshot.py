...
enabling the agent to understand and interact with web page elements.

Enhanced from CoPaw with research-specific role detection
(e.g., citation links, PDF viewers).
"""
...
def _create_tracker() -> dict[str, Any]:
    ...
    def get_key(role: str, name: Optional[str]) -> str:
    ...
    def track_ref(role: str, name: Optional[str], ref: str) -> None:
        key = get_key(role, name)
...
def build_role_snapshot_from_aria(
    ...
    *,
    ...
    Args:
        aria_snapshot: Raw aria snapshot text from Playwright.
        interactive: If True, only include interactive elements.
        compact: If True, remove structural nodes without refs.
        max_depth: Maximum nesting depth to include.

    Returns:
        Tuple of (formatted snapshot text, refs dict mapping ref IDs
        to role/name/nth info).
    """
    options = {
