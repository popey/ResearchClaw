...
Enhanced from CoPaw with additional file type magic detection
and research-friendly file operations.
"""
...
def safe_resolve_path(path_str: str, base_dir: str | None = None) -> Path:
    ...
    base_dir:
        Optional base directory to restrict paths within.
    ...
    ------
    ValueError
        If the path tries to escape the base directory.
    """
...
def _download_remote_to_path(url: str, local_file_path: Path) -> None:
    """Download url to local_file_path via wget, curl, or urllib.
...
# Magic bytes (prefix) -> suffix for file type detection.
...
async def download_file_from_url(
    ...
    """Download a file from URL to local download directory.

    Uses wget, curl, or urllib as fallbacks.

    Args:
        url: The URL of the file to download.
        filename: The filename to save. If not provided, will extract from
            URL or generate a hash-based name.
        download_dir: The directory to save files. Defaults to "downloads".
    ...
    try:
        ...
        # Fix generic extensions by detecting real file type
