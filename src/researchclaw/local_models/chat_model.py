...
Wraps any ``LocalBackend`` (llama.cpp, MLX) and presents it through the
agentscope ``ChatModelBase`` interface. Supports streaming, tool calling
via ``<tool_call>`` tags, and ``<think>`` reasoning extraction.
"""
...
class LocalChatModel(ChatModelBase):
    ...
    Wraps any ``LocalBackend`` (llama.cpp, MLX) and presents it through
    the agentscope ``ChatModelBase`` interface. Since backends are
    synchronous, inference runs in a thread executor for async compat.
    """
    ...
    async def _stream_response(
        ...
        **kwargs: Any,
        ...
        """Wrap synchronous streaming as async generator via queue."""
        loop = asyncio.get_running_loop()
