...
function ChatPage() {
  ...
  async function onSendChat() {
    ...
    try {
      ...
      setMessages((prev: ChatMessage[]) => [...prev, { role: 'assistant', content: res.response }])
    } catch (error) {
      setMessages((prev: ChatMessage[]) => [...prev, { role: 'assistant', content: `请求失败: ${String(error)}` }])
    } finally {
  ...
  return (
    <section className="panel">
      ...
      <div className="messages">
        {messages.length === 0 && <p className="muted">开始和 Scholar 对话：文献综述、实验设计、写作润色等。</p>}
        {messages.map((msg, idx) => (
          <div key={idx} className={`msg ${msg.role}`}>
            <strong>{msg.role === 'user' ? '你' : 'Scholar'}:</strong> {msg.content}
          </div>
        ))}
      </div>
      <div className="row">
        ...
        />
        <button onClick={onSendChat} disabled={!canSend}>{chatLoading ? '发送中...' : '发送'}</button>
      </div>
...
function PapersPage() {
  const [paperQuery, setPaperQuery] = useState('large language model reasoning')
  ...
  return (
    <section className="panel">
      ...
      <div className="row">
        <input
          value={paperQuery}
          onChange={(e: ChangeEvent<HTMLInputElement>) => setPaperQuery(e.target.value)}
          placeholder="输入检索主题"
        />
        <button onClick={onSearchPapers} disabled={paperLoading}>{paperLoading ? '检索中...' : '检索 ArXiv'}</button>
      </div>
      <div className="list">
        {papers.map((paper, idx) => (
          <div key={idx} className="card">
            <h3>{paper.title || 'Untitled'}</h3>
            {paper.id && <p><strong>ID:</strong> {paper.id}</p>}
            {paper.published && <p><strong>Published:</strong> {paper.published}</p>}
            {paper.authors && paper.authors.length > 0 && <p><strong>Authors:</strong> {paper.authors.join(', ')}</p>}
            {paper.summary && <p className="muted">{paper.summary.slice(0, 240)}...</p>}
          </div>
        ))}
      </div>
    </section>
...
function StatusPage() {
  ...
  async function onRefreshStatus() {
    ...
    try {
      ...
      setAgentInfo(`${s.agent_name} | running=${String(s.running)} | tools=${s.tool_count}`)
    } catch (error) {
  ...
  return (
    <section className="panel">
      ...
      <p><strong>Agent:</strong> {agentInfo}</p>
      {control && (
        <>
          <p><strong>Mode:</strong> {control.mode}</p>
          <p><strong>Uptime(s):</strong> {control.uptime_seconds}</p>
          <p><strong>Cron Jobs:</strong> {Array.isArray(control.cron_jobs) ? control.cron_jobs.length : 0}</p>
        </>
      )}
    </section>
  )
        ...
        {sessions.map((session: SessionItem) => (
          <div key={session.session_id} className="card">
            ...
            <div className="row">
              ...
              <button className="danger" onClick={() => onDelete(session.session_id)}>删除</button>
            </div>
          </div>
        ))}
      </div>
      {selected && <pre className="pre">{JSON.stringify(selected, null, 2)}</pre>}
    </section>
  )
}
        ...
        {jobs.map((job: CronJobItem) => (
          <div key={job.name} className="card row spread">
            <div>
              ...
              <p className="muted">
                interval: {job.interval_seconds}s | running: {String(job.running)}
              </p>
            </div>
            <button onClick={() => onToggle(job.name, !job.enabled)}>{job.enabled ? '禁用' : '启用'}</button>
          </div>
        ))}
      </div>
    </section>
...
function HeartbeatPage() {
  ...
  return (
    <section className="panel">
      ...
      {heartbeat && <pre className="pre">{JSON.stringify(heartbeat, null, 2)}</pre>}
    </section>
  )
}
        ...
        {skills.map((skill: SkillItem, idx: number) => {
          ...
          return (
            <div key={skillName} className="card row spread">
              ...
              <button onClick={() => onToggle(skillName, isActive)}>{isActive ? '禁用' : '启用'}</button>
            </div>
          )
        })}
      </div>
  ...
  return (
    <section className="panel">
      ...
      <div className="row wrap">
        <input value={newKey} onChange={(e: ChangeEvent<HTMLInputElement>) => setNewKey(e.target.value)} placeholder="client key" />
        <input value={newName} onChange={(e: ChangeEvent<HTMLInputElement>) => setNewName(e.target.value)} placeholder="display name" />
        <input value={newCommand} onChange={(e: ChangeEvent<HTMLInputElement>) => setNewCommand(e.target.value)} placeholder="command (e.g. npx)" />
      ...
      <div className="list">
        {clients.map((item: McpClientItem) => (
          <div key={item.key} className="card">
            ...
            <p><strong>Enabled:</strong> {String(item.enabled ?? true)}</p>
            <div className="row">
              <button onClick={() => onToggle(item.key)}>{item.enabled ? '禁用' : '启用'}</button>
...
function WorkspacePage() {
  ...
  const [profile, setProfile] = useState<{ exists: boolean; content: string; path?: string } | null>(null)
  ...
  return (
    <section className="panel">
      ...
      {workspace && <pre className="pre">{JSON.stringify(workspace, null, 2)}</pre>}
      {profile && (
        <>
          <h3>PROFILE.md</h3>
          {!profile.exists ? <p className="muted">未找到 PROFILE.md</p> : <pre className="pre">{profile.content}</pre>}
        </>
      )}
    </section>
  )
...
function AgentConfigPage() {
  const [config, setConfig] = useState<AgentRunningConfig>({ max_iters: 50, max_input_length: 128000 })
  ...
  return (
    <section className="panel">
      ...
      <div className="list">
        <div className="card">
          <p><strong>max_iters</strong></p>
          <input
            type="number"
            value={config.max_iters}
            onChange={(e: ChangeEvent<HTMLInputElement>) => setConfig((prev: AgentRunningConfig) => ({ ...prev, max_iters: Number(e.target.value) || 1 }))}
          />
        </div>
        <div className="card">
          <p><strong>max_input_length</strong></p>
          <input
            type="number"
            value={config.max_input_length}
            onChange={(e: ChangeEvent<HTMLInputElement>) => setConfig((prev: AgentRunningConfig) => ({ ...prev, max_input_length: Number(e.target.value) || 1000 }))}
          />
        </div>
      </div>
    </section>
    ...
    <section className="panel">
      ...
      <div className="list">
        {providers.map((provider, idx) => (
          <div key={idx} className="card">
            <p><strong>{provider.name || provider.id || '-'}</strong></p>
            <p><strong>Type:</strong> {provider.provider_type || provider.type || '-'}</p>
            {provider.model_name && <p><strong>Model:</strong> {provider.model_name}</p>}
      ...
      <div className="list">
        {models.map((model, idx) => (
          <div key={idx} className="card">
            <p><strong>{model.name || model.model_name || '-'}</strong></p>
