...
export async function searchArxiv(query: string, maxResults = 8): Promise<PaperItem[]> {
...
export async function getStatus(): Promise<{ running: boolean; agent_name: string; tool_count: number }> {
...
export async function getSessionDetail(sessionId: string): Promise<any> {
  const res = await fetch(`/api/control/sessions/${encodeURIComponent(sessionId)}`)
...
export async function toggleCronJob(name: string, enabled: boolean): Promise<void> {
  const action = enabled ? 'enable' : 'disable'
  const res = await fetch(`/api/control/cron-jobs/${encodeURIComponent(name)}/${action}`, {
    method: 'POST',
  })
...
export async function createMcpClient(key: string, body: Omit<McpClientItem, 'key'>): Promise<void> {
...
export async function updateMcpClient(key: string, body: Omit<McpClientItem, 'key'>): Promise<void> {
...
export async function getWorkspaceProfile(): Promise<{ exists: boolean; content: string; path?: string }> {
...
export async function updateAgentRunningConfig(config: AgentRunningConfig): Promise<AgentRunningConfig> {
  const res = await fetch('/api/agent/running-config', {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json' },
...
export function appendMessage(messages: ChatMessage[], next: ChatMessage): ChatMessage[] {
